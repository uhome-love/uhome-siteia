...
export function BairrosRecomendados() {
  ...
  useEffect(() => {
    async function load() {
      // Get top bairros with count and avg price
    ...
    <div className="py-8">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-body text-base font-bold text-foreground">
          Bairros recomendados para comprar imóveis em Porto Alegre
        </h3>
      ...
      <div className="hidden sm:grid grid-cols-4 gap-3">
        {visible.map((b) => (
          <Link
            ...
            className="group flex flex-col justify-between rounded-xl border border-border bg-card p-5 transition-all hover:border-primary/40 hover:shadow-sm"
          >
            <div>
            ...
            <ChevronRight className="absolute right-4 top-5 h-4 w-4 text-muted-foreground/50 group-hover:text-primary transition-colors hidden group-hover:block" />
          </Link>
        ))}
      </div>
